...
impl BackendInner for OnnxBackend {
    ...
    fn load(&mut self, builders: &[&[u8]], target: ExecutionTarget) -> Result<Graph, BackendError> {
        ...
        let used_execution_providers = match target {
            ExecutionTarget::Cpu => vec![
                ort::CPUExecutionProvider::default().build()
            ],
            ExecutionTarget::Gpu => vec![
            ...
            ExecutionTarget::Tpu => unimplemented!("ONNXRuntime does not support TPU execution targets"),
        };
...
impl BackendExecutionContext for ONNXExecutionContext {
    ...
    fn compute(&mut self) -> Result<(), BackendError> {
        ...
        for i in 0..self.outputs.len() {
            let raw: (Vec<i64>, &[f32]) = res[i].try_extract_raw_tensor()?;
